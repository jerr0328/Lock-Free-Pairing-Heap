...
Our \texttt{insert} and \texttt{decreaseKey} operators implement
...
Suppose we have $n$ operations $\{\sigma_1, \dots, \sigma_n\}$ (these are either
calls to \texttt{insert} or \texttt{decreaseKey}) concurrently executing. Let
...
An \texttt{decreaseKey} has three effects: either the new node becomes (or is already)
...
to modify the same node in the heap do not occur.
...
we additionally have $n < r$. Say that $r' < n < r$.
...
thread overrides the parent field modification of another thread. If the
first 
